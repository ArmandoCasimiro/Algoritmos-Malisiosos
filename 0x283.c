...
void fatal(char *message) {
    ...
    strncpy(error_message, "[!!] Fatal Error ", sizeof(error_message) - 1); // Usa strncpy en lugar de strcpy
    strncat(error_message, message, sizeof(error_message) - strlen(error_message) - 1); // Asegura que no haya desbordamiento
...
int main(int argc, char *argv[]) { 
    ...
    strncpy(buffer, argv[1], 99); // Copy into buffer.
